...
#
# 对应的相同请求 curl 形式：
#   curl -X POST '<relay-base-url>/v1/responses' \
...
#   OPENAI_BASE_URL=https://your-relay.example       # 你的中转域名
#   OPENAI_API_KEY=<relay-token>                     # 中转站颁发的 token
...
**关于中转站**：
- OpenAI 兼容的中转站或自建反代通常保留 `/v1/images/...` 这套 path，只换 host 和 token。直接把 `OPENAI_BASE_URL` 设成它们的 base host 就行，仍用 `IMAGE_GEN_API_PROVIDER=gpt-image`。
- 部分中转站只走 Responses API（`/v1/responses` + 流式 + 尺寸进模型名），用 `IMAGE_GEN_API_PROVIDER=gpt-image-responses`，详见上面 `.env.local` 注释段。判断方法：看官方 curl 是不是 `/v1/responses` + `model: 'gpt-image-1024x1024'` 这种形态。
...
# 切到 Responses-API 中转站（只暴露 /v1/responses 的反代）
cat > .env.local <<'EOF'
IMAGE_GEN_API_PROVIDER=gpt-image-responses
