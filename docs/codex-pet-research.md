...
|------|-----------|------------|
| 核心流程 | AI 生成 → 切帧 → 拼接图集 | AI 生成 → 切帧 → 拼接图集 |
| 图集概念 | 单张 8×9 atlas | 单张 spritesheet |
| 风格要求 | 像素风 chibi，有明确否定约束 | 通过 `prompt_style` 定义风格 |
| 导出格式 | pet.json + spritesheet.webp | TexturePacker / Godot .tres / Aseprite 等 |
| 动画状态 | 9 种固定状态 | 自定义动作类型 (walk, idle, attack 等) |

### 4.2 关键差异

| 维度 | Codex Pet | SpriteBrew |
|------|-----------|------------|
| **AI 后端** | OpenAI $imagegen (内置 skill) | Retro Diffusion API (第三方) |
| **背景去除** | Chroma key + 脚本（确定性） | AI 生成时自带透明背景 |
| **帧提取** | Connected components（智能） | 固定格子切分 (SlicerConfig) |
| **图集规格** | 固定 1536×1872 (8×9×192×208) | 用户自定义 (宽/高/列/帧数) |
| **Prompt 工程** | 极其精细，每行有否定约束列表 | 相对简单，依赖 prompt_style 映射 |
| **角色一致性** | Identity Lock + canonical base 参考 | 无显式机制 |
| **QA 验证** | 三层自动化 + 人工联系表 | 无自动化 QA |
| **并行生成** | 子代理并行 9 行 | 单次 SSE 串行 |
| **用途** | 终端桌面宠物 | 游戏角色资产生成 |
...
**建议**：在 SpriteBrew 的 `api/generate/route.ts` 中，为 Animate 模式增加"身份锁定"参数——用户上传的角色图自动作为 reference，prompt 中注入 identity lock 段落。
...
**建议**：将 chroma key 去背景作为一个可选后处理步骤引入 SpriteBrew，特别是当上游 API 不支持透明输出时。

#### (3) Connected Components 智能帧提取
...
**建议**：在 SpriteBrew 的切分工具中增加"智能切分"模式，作为手动配置的替代方案。
...
#### (5) 布局引导图 (Layout Guides)

`prepare_pet_run.py` 会生成每行动画的布局引导图——显示帧数、格子大小、安全边距、中心十字线。这个引导图作为"不可见参考"传给 AI，帮助 AI 正确放置角色。

**建议**：在 SpriteBrew 的"Animate My Character"模式中，自动生成类似的布局引导图附加到 AI 请求中，确保生成的帧间距合理。

#### (6) 子代理并行生成

Codex Pet 在 base 完成后并行生成 9 行动画，大幅缩短总生成时间。SpriteBrew 目前是单次 SSE 串行。

**建议**：中期考虑引入并行生成。在 Next.js 中可以用 `Promise.all` 同时发起多个 API 请求。需要注意 token 消耗。
...
1. **Identity Lock for Animate Mode**：在 Animate My Character 流程中，用户上传的角色图作为 `canonical base`，每个动画帧的 prompt 中注入 identity lock 段落。

2. **否定约束注入**：根据动画类型，在 prompt 中自动附加"不要画什么"规则。例如 walk 动画禁止速度线和灰尘，attack 动画禁止 UI 元素等。

3. **风格基线定义**：参考 `DIGITAL_PET_STYLE` 的写法，为 SpriteBrew 的每种 prompt_style 定义一段明确的"是什么/不是什么"风格描述。

### Phase 2: 后处理管线（短期）

4. **Chroma Key 去背景选项**：引入可选的 chroma key 去背景步骤，当上游 API 不支持透明输出时启用。

5. **智能切分模式**：在 SlicerConfig 中增加"自动检测"模式，使用连通域分析。

6. **导出前自动验证**：在 export 流程中增加几何验证（尺寸、非空、透明度）和联系表预览。

### Phase 3: 生成优化（中期）

7. **布局引导图**：Animate 模式自动生成布局引导图，附加到 AI 请求中。
